...
import { tool } from "ai"
import { z } from "zod"
import { projectContentRoot } from "../runtime/paths"
...
export function buildAgentTools(projectSlug: string) {
  const workspaceRoot = projectContentRoot(projectSlug)
  ...
  const bashTool = tool({
    ...
    execute: async ({
      ...
      if (background) {
        const job = await processManager.createAndSpawn(projectSlug, command, workDir)
        return {
          jobId: job.id,
          pid: job.pid,
          status: job.status,
